...
# ===== 1 =====
h1("1. 这套安装包做了什么")
p("- 支持选择 npm 镜像源（淘宝镜像），国内用户安装速度大幅提升。")
...
model_table = Table([
    ["模式", "需要填写", "说明", "网络要求"],
    ["OpenAI", "apiKey, modelRef", "默认 openai/gpt-5.4", "需要科学上网"],
    ["OpenRouter", "apiKey, modelRef", "默认 openrouter/auto", "需要科学上网"],
    ["Moonshot", "apiKey, modelRef", "默认 moonshot/kimi-k2.5", "国内直连"],
    ["自定义兼容", "baseUrl, apiKey, providerId, customModelId", "中转站 / 自建接口 / 本地推理", "取决于提供商"],
], colWidths=[25*mm, 50*mm, 50*mm, 33*mm])
...
E.append(sp())
warn("OpenAI 和 OpenRouter 的 API 服务器在海外，安装和使用过程中都需要科学上网（梯子/VPN）。国内用户建议选择 Moonshot 或国内自定义提供商。")
E.append(sp())
...
# ===== 7 =====
h1("7. npm 镜像源选择")
p("安装向导会询问你使用哪个 npm 镜像源。国内用户强烈建议选择淘宝镜像，安装速度可提升数倍。")
mirror_table = Table([
    ["选项", "地址", "适用场景"],
    ["默认 (Default)", "registry.npmjs.org", "海外用户或已有科学上网"],
    ["淘宝镜像 (China mirror)", "registry.npmmirror.com", "国内用户，无需梯子即可加速"],
], colWidths=[45*mm, 55*mm, 58*mm])
mirror_table.setStyle(TableStyle([
    ("FONTNAME", (0,0), (-1,-1), "ZH"), ("FONTSIZE", (0,0), (-1,-1), 9.5),
    ("BACKGROUND", (0,0), (-1,0), BG_HEAD),
    ("GRID", (0,0), (-1,-1), 0.5, LINE), ("VALIGN", (0,0), (-1,-1), "TOP"),
]))
E.append(mirror_table)
tip("安装器会在安装前自动设置 npm registry，安装完成后恢复你原来的源设置，不会影响你其他项目。")
tip("预设配置安装时，在 installer-config.local.json 的 openclaw.npmMirror 字段填入 https://registry.npmmirror.com 即可。留空则使用默认源。")
E.append(sp())
